...
public:
	...
	~Token()
	{
		...
		if (var_integer && var_integer != "Nul") {
			delete(var_integer);
		}
	}
};
...
public:
	...
	Token* ListToken::setInteger(char* name, char* new_interger)
	{
		...
		if (!strcmp(temp->token->value, name))
		{
			temp->token->var_integer = new char[strlen(new_interger) + 1];
			temp->token->var_integer = strcpy(temp->token->var_integer,new_interger);
			return root->token;
		}
		while (temp->next != NULL)
		{
			...
			{
				temp->next->token->var_integer = new char[strlen(new_interger) + 1];
				temp->next->token->var_integer = strcpy(temp->next->token->var_integer, new_interger);
...
public:
	...
	Token* get_next_token()
	{
		...
		if (isalpha(this->Curr_Char))
		{
			...
			result[count] = '\0';
			if (result[0] == 'N' && result[1] == 'u' && result[2] == 'l'&& result[3] == '\0')
			{
				free(result);
				return curr_token = new Token(INT, "Nul", NONE);
			}
			else
			{
		...
		if (isdigit(this->Curr_Char))
		{
			...
			result[count] = '\0';
			curr_token = new Token(INT, result, NONE);
			free(result);
			return curr_token;
		}
		if (this->Curr_Char == '*')
...
private:
	...
	char* ONPCalc(QueueToken queue)
	{
		...
		while (!queue.isEmpty())
		{
			...
			else
			{
				op1 = stack.pop();
				op2 = stack.pop();
				if (op1->type == INT && op1->value!="Nul") op1_int = true;
				if (op2->type == INT&& op2->value!="Nul") op2_int = true;
				calc = do_calculations(item, op1, op2);
				stack.push(calc);
				if (op1_int == true) delete op1;
				if (op2_int == true) delete op2;
				op1_int = false;
				op2_int = false;
				if (operations == Op_limit) break;
			}
	...
	Token* neg_operator(Token* op1)
	{
		...
		{
			if (strcmp(op1->value, "Nul") != 0)
			{
				delete op1;
				return new Token(INT, "Nul", NONE);
			}
			else  
			{
				delete op1;
				return new Token(INT, "0", NONE);
			}
		}
	}
	Token* unary_operator(Token* op1)
	{
		...
		int new_val = 0;
		int digits = 0;
		if(op1->type==VAR &&strcmp(op1->var_integer, "Nul") == 0)  return new Token(INT, "Nul", NONE);
		if (op1->type==INT && strcmp(op1->value, "Nul") == 0) return new Token(INT, "Nul", NONE);
		else
		{
			if (op1->type == VAR) {
				operations++;
	...
	Token* do_calculations(Token* item, Token* op1, Token* op2)
	{
		...
		{
			if ( strcmp(op2->value, "Nul") == 0 ) return new Token(INT, "Nul", NONE);
		}
		...
		if (item->type == MUL) {
			...
			{
				digits = checkDigits(res);
				result = (char*)malloc(digits*sizeof(char) + 1);
			}
			itoa(res, result, 10);
			temp = new Token(INT, result, NONE);
			free(result);
